...
def getUnclosedCoursesByIntitule(intitule)
  if vExistsPromo(intitule).length.zero?
    ...
    request_object = OpenConnectBdd.prepare('SELECT c.intitule AS initule, c.heure AS heure, c.date AS date, c.salle AS salle, m.intitule AS matiere, po.intitule AS promo FROM cours c JOIN matiere m ON c.id_matiere=m.id_matiere JOIN promo po ON c.id_promo=po.id_promo WHERE c.status = 0 AND po.intitule = ?')
